...
fn compute_points(duel: [u64; 2]) -> u64 {
    ...
    let match_result: i64 =
        (duel[1] as i64 - duel[0] as i64).rem_euclid(3);
    if match_result == 1 {
        // win
        result += 6;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // dbg!(&input_list);
    let list_points = input_list
        .map(|duel: [&str; 2]| duel.map(letter_to_points));
    // dbg!(&list_symbols);
    let part1: u64 =
        list_points.map(|duel| compute_points(duel)).sum();
    ...
    fn generate_symbols(duel: Split<&str>) -> [u64; 2] {
        let duel: [&str; 2] =
            duel.collect::<Vec<&str>>().try_into().unwrap();
        let opponent_shape = letter_to_points(duel[0]);

        match duel[1] {
            "X" => [
                opponent_shape,
                (opponent_shape + 1).rem_euclid(3) + 1,
            ], // lose
            "Y" => [opponent_shape, opponent_shape], // draw
            "Z" => [
                opponent_shape,
                opponent_shape.rem_euclid(3) + 1,
            ], // win
            _ => panic!(),
        }
    }
    ...
    // dbg!(&list_symbols);
    let part2: u64 =
        list_symbols.map(|duel| compute_points(duel)).sum();
