...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut elves_total_cal_list = elves_carrying_list
        .iter()
        .map(|e| e.iter().sum())
        .collect::<Vec<u64>>();
    let max_carrying: u64 =
        *elves_total_cal_list.iter().max().unwrap();
    println!("Partie 1 : {max_carrying}");

    // -------- Part 2 ---------
    elves_total_cal_list.sort();

    let result_part_2: u64 = elves_total_cal_list
        [elves_total_cal_list.len() - 3
        ..elves_carrying_list.len()]
        .iter()
        .sum();
